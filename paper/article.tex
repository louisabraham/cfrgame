...
\begin{abstract}
%% Text of abstract
In this study, we explore models where participants strategically choose their risk levels and earn rewards in line with their chosen risks, mimicking real-world competition in fields ranging from market-share-seeking firms to banks and insurance companies vying for more customers. 

Our analysis starts with a simple normal form game with two players in a continuous action space. We confirm the existence of a unique Nash equilibrium and provide an analytical solution. We then extend the analysis to multi-player scenarios to explore the Nash equilibrium in more complex settings. Despite challenges, we show that our Competition for Risk (CfR) game model exhibits a unique symmetric equilibrium for multiple players, introducing a new numerical algorithm for its calculation. Additionally, we thoroughly examine the equilibrium's asymptotic behavior. With this foundation, we incorporate additional real-world factors such as market frictions and risk correlations among firms. Our experiments confirm that the Nash equilibrium in our model also serves as a correlated equilibrium. We also introduce the reader to the use of regret matching variants as efficient algorithms for approximating the Nash equilibrium. Our methodology and code are open-source and compatible with any normal form game\footnote{available at \identifier{\url{https://github.com/louisabraham/cfrgame}}}.

Further, we discuss experimental findings on the effects of market frictions and risk correlations on strategic risk-taking. We find that market frictions, like transaction costs and imperfect information, significantly influence player incentives. These frictions typically lower the average risk taken while enhancing the total payoff. In inefficient markets, increasing failure penalties can further optimize payoffs. Additionally, we find that risk correlation significantly affects risk-taking: players take less risk when risks are positively correlated and more risk when negatively correlated.

Our findings extend beyond strategic risk-taking, offering insights for policy design in various sectors. Notably, our model exhibits ties to the Diamond-Dybvig framework, albeit with broader applicability. Using our model, we explore how policy measures, such as increasing transparency or easing contract transferability, can influence risk-taking and its rewards.

Additionally, we contribute methodologically by advocating for algorithm use in economics. While simplified models traditionally favored in economics allow for analytical solutions, algorithmic solvers can handle more complex models without closed-form solutions or requiring data inputs. Thus, we shift focus from finite games to continuous action set games.
...
\begin{figure}[htbp]
  ...
  \caption{Approximate and analytical solutions for $P=1, \rho=0, \tau=0$. Actions were discretized to a grid of $2^{12}$ values. The approximate solution was computed ]}
  \label{fig:rm-sol}  
\end{figure}
